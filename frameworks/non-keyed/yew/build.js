(async () => {
  ...
  ['stable', buildToolchain].forEach(toolchain => {
    if (spawnSync(rustup, ['run', toolchain, 'rustc', '--version']).status !== 0) {
  ...
  const frameworkVersion = cargoMetadataObject.packages.find(({ name }) => name === path.basename(__dirname)).version
  let packageJsonObject = JSON.parse(fs.readFileSync('package.json', 'utf8'))
  packageJsonObject['js-framework-benchmark']['frameworkVersion'] = frameworkVersion
